!(function (t) {
	 ...
	 */ !(function () {
		...
		function n(t) {
			const e = t._context.chart.config._config;
			return (
				!(!e.options || !e.options.plugins) &&
				!!e.options.plugins.labels
			);
		}
		['pie', 'doughnut', 'polarArea', 'bar'].forEach(function (t) {
			...
			(o.prototype.getPercentage = function (t, e, o) {
				...
				if (
					...
					if (!this.barTotal[o]) {
						...
						for (
							let t = 0;
							t < this.chart.data.datasets.length;
							++t
						)
							this.barTotal[o] +=
								this.chart.data.datasets[t].data[o];
					}
					n = (t.data[o] / this.barTotal[o]) * 100;
				} else
					n =
						(e.circumference /
							this.chart.config.options.circumference) *
						100;
				return (
					...
					this.options.showActualPercentages ||
						('bar' === this.chart.config.type &&
							(this.totalPercentage =
								this.barTotalPercentage[o] || 0),
						(this.totalPercentage += n),
						this.totalPercentage > 100 &&
							((n -= this.totalPercentage - 100),
							(n = parseFloat(
								n.toFixed(this.options.precision)
							))),
						'bar' === this.chart.config.type &&
							(this.barTotalPercentage[o] =
								this.totalPercentage)),
					(this.percentage = n),
					n
				);
			...
			(o.prototype.getArcRenderInfo = function (t, e) {
				...
				return (
					(i += (s - (this.measureLabel(e).width / o + i)) / 2),
					{
						radius: o,
						startAngle: i,
						endAngle: s,
			...
			(o.prototype.drawable = function (t, e, o) {
				...
				if (this.options.arc)
					return o.endAngle - o.startAngle <= o.totalAngle;
				{
					const n = this.measureLabel(e),
						...
						r = o.y - n.height / 2,
						a = o.y + n.height / 2;
					return 'outside' === this.options.position
			...
			(o.prototype.measureLabel = function (t) {
					...
					return { width: t.width, height: t.height };
				{
					...
					return {
						width: e,
						height: this.options.fontSize * o.length,
					};
				}
			}),
			...
			Chart.register({
				id: 'labels',
				beforeDatasetsUpdate: function (t, i, s) {
					if (!e[t.config.type] || !n(s)) return;
					s.length || (s = [s]);
					const r = s.length;
					(t._labels && r === t._labels.length) ||
						(t._labels = s.map(function () {
							return new o();
						}));
					let a = !1,
						h = 0;
					for (let e = 0; e < r; ++e) {
						const o = t._labels[e];
						if (
							(o.setup(t, s[e]), 'outside' === o.options.position)
						) {
							a = !0;
							const t =
								1.5 * o.options.fontSize +
								o.options.outsidePadding;
							t > h && (h = t);
						}
					}
					a && ((t.chartArea.top += h), (t.chartArea.bottom -= h));
				},
				afterDatasetUpdate: function (t, o, i) {
					e[t.config.type] &&
						n(i) &&
				...
				afterDatasetsDraw: function (t, o, i) {
					e[t.config.type] &&
						n(i) &&
						t._labels?.forEach(function (t) {
							t.render();
